...
# 为 Open Code 安装 awesun-remote-control skill
# 作者：AI Assistant
# 日期：2026年3月5日
...
echo -e "${BLUE}=== Open Code - Awesun Remote Control Skill 安装程序 ===${NC}"
echo

# 检查 Open Code 是否存在
if ! command -v opencode &> /dev/null; then
    echo -e "${RED}❌ 错误：未找到 opencode 命令${NC}"
    echo -e "${YELLOW}请确保已正确安装 Open Code 并添加到 PATH${NC}"
    exit 1
fi

echo -e "${GREEN}✅ 找到 Open Code${NC}"
...
# 获取 Open Code 配置目录
echo "🔍 获取 Open Code 配置目录..."
if [[ "$OSTYPE" == "darwin"* ]]; then
    OPENCODE_CONFIG_DIR="$HOME/Library/Application Support/Open Code"
elif [[ "$OSTYPE" == "linux-gnu"* ]]; then
...
# 验证安装
echo "✅ 验证安装..."
if [ -f "$TARGET_DIR/SKILL.md" ] && [ -x "$TARGET_DIR/bin/mcp2skill" ] && [ -f "$TARGET_DIR/skill.json" ]; then
...
echo -e "${YELLOW}3. 重启 Open Code 以加载新的 skill${NC}"
echo -e "${YELLOW}4. 在 Open Code 设置中启用 awesun-remote-control skill${NC}"
echo -e "${YELLOW}5. 在 Open Code 中即可使用向日葵远程控制功能${NC}"
echo
