# 为 Open Code 安装 awesun-remote-control skill (Windows PowerShell版本)
# 作者：AI Assistant
# 日期：2026年3月5日
...
if ($Help) {
    Write-Host @"
Open Code - Awesun Remote Control Skill 安装程序 (Windows)

用法:
    .\install-opencode.ps1 [-Force] [-Help]
...
try {
    Write-ColorOutput "=== Open Code - Awesun Remote Control Skill 安装程序 (Windows) ===" "Blue"
    Write-Host ""
    ...
    # 检查 Open Code 是否安装
    Write-ColorOutput "🔍 检查 Open Code..." "Blue"
    $openCodePath = $null
    
    # 尝试多个可能的安装位置
    $possiblePaths = @(
        "$env:LOCALAPPDATA\Programs\Open Code\Open Code.exe",
        "$env:PROGRAMFILES\Open Code\Open Code.exe",
        "${env:PROGRAMFILES(X86)}\Open Code\Open Code.exe",
    ...
    # 也检查PATH中是否有opencode命令
    $openCodeCmd = Get-Command "opencode" -ErrorAction SilentlyContinue
    
    if (-not $openCodePath -and -not $openCodeCmd) {
        Write-ColorOutput "❌ 错误：未找到 Open Code" "Red"
        Write-ColorOutput "请下载并安装 Open Code" "Yellow"
        exit 1
    }
    
    Write-ColorOutput "✅ 找到 Open Code" "Green"
    ...
    # 获取 Open Code 配置目录
    Write-ColorOutput "🔍 获取 Open Code 配置目录..." "Blue"
    $openCodeConfigDir = "$env:APPDATA\Open Code"
    
    # 如果标准目录不存在，尝试其他可能位置
    if (-not (Test-Path $openCodeConfigDir)) {
        $altPaths = @(
            "$env:LOCALAPPDATA\Open Code",
    ...
    # 创建 Open Code skill 配置文件
    Write-ColorOutput "📝 创建 skill 配置文件..." "Blue"
    $skillConfigContent = @"
{
  "name": "awesun-remote-control",
  "version": "1.0.0",
  "description": "向日葵远程控制 Skill - 提供设备管理、远程桌面、远程命令行等功能",
  "author": "OrayDev",
  "license": "MIT",
  "keywords": ["remote-control", "desktop", "awesun", "mcp"],
  "main": "bin/mcp2skill",
...
"@
    ...
    $skillConfigPath = Join-Path $targetDir "skill.json"
    $skillConfigContent | Out-File -FilePath $skillConfigPath -Encoding UTF8
    ...
    if ((Test-Path $skillMd) -and (Test-Path $mcpToolPath) -and (Test-Path $skillConfigPath)) {
    ...
    Write-ColorOutput "3. 重启 Open Code 以加载新的 skill" "Yellow"
    Write-ColorOutput "4. 在 Open Code 设置中启用 awesun-remote-control skill" "Yellow"
    Write-ColorOutput "5. 在 Open Code 中即可使用向日葵远程控制功能" "Yellow"
    Write-Host ""
